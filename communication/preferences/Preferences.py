...
from communication.preferences.CriterionDef import criterion_find, criterion_name, CriterionValue
...
class Preferences:
    ...
    def most_preferred(self, item_list):
        ...
        return item_list[0]
    def n_most_preferred(self, item_list, n):
        """Returns the n most preferred item from a list."""
        item_list.sort(key=lambda x: x.get_score(self), reverse=True)
        return item_list[:n]
...
if __name__ == "__main__":
    ...
    preferences.set_criterion_name_list(
        [criterion_find(column) for column in dataset.columns if column != "COURSE"]
    )

    # Add values
    items = []
    for _, row in dataset.iterrows():
        new_item = Item(row["COURSE"], "")
    ...
    print(items[0])
    print(items[1])
    print(items[0].get_value(preferences, criterion_find("DIFFICULTY")))
    print(
        preferences.is_preferred_criterion(
            criterion_find("DIFFICULTY"), criterion_find("QUALITY")
        )
    )
